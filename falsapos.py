...
class Falsapos:
    ...
    def generar_tabla(self):
        ...
        while(xe!=self.raiz):
          ...
          if(xe == round((xp*fx(xn)-xn*fx(xp))/(fx(xn)-fx(xp)), 6)):
              break
         
          #xe = round(xn + ((xp-xn)/2), 6)
          xe = round((xp*fx(xn)-xn*fx(xp))/(fx(xn)-fx(xp)),6)
        ...
        print(tabla)
        #Mostramos la recta
        mostrar_raiz(fx,(xe,int(fx(xe))),"falsa posición")
...
if __name__ == "__main__":
    ...
    metodo = Falsapos(args.funcion,args.raiz,args.xn,args.xp)
    metodo.inicio()